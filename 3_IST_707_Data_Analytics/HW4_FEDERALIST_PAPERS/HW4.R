...
summary(kmeansFIT_2$iter)

############## Distance Measures ######################
getDistMethods()

# # # m <- m[1:2, 1:3]
distMatrix_E <- dist(filteredDtmMatrix_norm, method="euclidean")
#print(distMatrix_E)
...
distMatrix_Mi_norm <- dist(filteredDtmMatrix_norm, method="minkowski", p=5)
print(distMatrix_Mi_norm)
...
##  Minkowski for Normalized Matrix
distMatrix_Mi_norm <- dist(filteredDtmMatrix_norm, method="minkowski", p=3)
groups_Mi_n <- hclust(distMatrix_Mi_norm,method="ward.D")
plot(groups_Mi_n, cex=0.9, hang=-1)
rect.hclust(groups_Mi_n, k=5)
...
ggplot(dfSeq, aes(x=Author,fill=em,group=em,color=em))+
